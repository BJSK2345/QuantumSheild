// ---------------------------------------------------------------------------
// scanner.js — Rule-based cryptography risk scanner.
//
// Each rule is a regex + metadata. The scanner walks the rules over the text,
// records every match with a line/column, and returns structured findings.
//
// Categories:
//   quantum-vulnerable — broken by a large quantum computer (Shor's algorithm)
//   classically-weak   — already weak/broken on classical hardware
//   bad-practice       — risky handling regardless of the algorithm
// ---------------------------------------------------------------------------
...
// Risk → numeric weight (used by scoring.js).
...
export const RULES = [
  ...
  // --- Hardcoded private keys / secrets (critical) -------------------------
  {
...
/**
 * Scan a single source unit.
 * @param {{ filename?: string, content: string }} input
 * @returns {{ filename: string, findings: Array }}
 */
export function scanContent({ filename = 'pasted-input', content = '' }) {
  ...
  // Sort by severity then location for a clean UI.
  findings.sort(
